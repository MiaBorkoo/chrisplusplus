...
void FileService::listSharedFiles(int page, int pageSize) {
    std::cout << "📋 FILESERVICE: listSharedFiles called with page=" << page << ", pageSize=" << pageSize << std::endl;
    ...
    std::cout << "About to call m_client->sendRequest for /api/files/shares/received" << std::endl;
    m_client->sendRequest("/api/files/shares/received", "GET", payload);
    std::cout << "m_client->sendRequest for shared files completed" << std::endl;
}

void FileService::grantAccess(const QString& fileId, const QString& username) {
    std::cout << "🤝 FILESERVICE: grantAccess called for fileId: " << fileId.toStdString() << std::endl;
    
    // SECURE PATH: Use SecureFileHandler if available
    if (m_secureHandler && m_secureHandler->isInitialized()) {
        std::cout << "🔐 FILESERVICE: Using SECURE grant access path" << std::endl;
        
        bool success = m_secureHandler->shareFileSecurely(fileId, username, m_authToken);
        
        if (success) {
            std::cout << "✅ FILESERVICE: Secure file sharing completed successfully!" << std::endl;
            emit accessGranted(true, fileId, username);
        } else {
            std::cout << "❌ FILESERVICE: Secure file sharing failed" << std::endl;
            reportError("Secure file sharing failed");
            emit accessGranted(false, fileId, username);
        }
        return;
    }
    
    // FALLBACK PATH: Use legacy Client with proper API structure
    ...
    // Create proper FileShareRequest structure as expected by the API
    QJsonObject payload;
    payload["file_id"] = fileId;
    payload["recipient_username"] = username;
    
    // TODO: These should be generated by the secure system
    // For now, using placeholders to match API structure
    payload["encrypted_data_key"] = ""; // Base64 encoded encrypted DEK
    payload["share_grant_hmac"] = "";   // HMAC for integrity
    payload["share_chain_hmac"] = "";   // Chain of trust HMAC
    
    // Optional fields
    // payload["expires_at"] = null;
    // payload["max_downloads"] = null;

    std::cout << "📤 FILESERVICE: Sending FileShareRequest for fileId: " << fileId.toStdString() << std::endl;
    m_client->sendRequest("/api/files/share", "POST", payload);
}

void FileService::revokeAccess(const QString& fileId, const QString& username) {
    std::cout << "🚫 FILESERVICE: revokeAccess called for fileId: " << fileId.toStdString() << std::endl;
    
    // SECURE PATH: Use SecureFileHandler if available
    if (m_secureHandler && m_secureHandler->isInitialized()) {
        std::cout << "🔐 FILESERVICE: Using SECURE revoke access path" << std::endl;
        
        bool success = m_secureHandler->revokeFileAccess(fileId, username, m_authToken);
        
        if (success) {
            std::cout << "✅ FILESERVICE: Secure access revocation completed successfully!" << std::endl;
            emit accessRevoked(true, fileId, username);
        } else {
            std::cout << "❌ FILESERVICE: Secure access revocation failed" << std::endl;
            reportError("Secure access revocation failed");
            emit accessRevoked(false, fileId, username);
        }
        return;
    }
    ...
    // TODO: We need the actual share_id to revoke a specific share
    // For now, this is a placeholder implementation
    QString shareId = "placeholder_share_id";
    QJsonObject payload;

    std::cout << "📤 FILESERVICE: Revoking share for fileId: " << fileId.toStdString() << std::endl;
...
void FileService::handleFileListResponse(const QJsonObject& data, bool isSharedList) {
    std::cout << "📋 FILESERVICE: Processing file list response (isSharedList=" << isSharedList << ")" << std::endl;
    ...
    if (isSharedList) {
        // For shared files, the API returns an array directly, not wrapped in an object
        // Check if the data contains an array directly or if it's wrapped
        if (data.contains("shared_files")) {
            // Wrapped format (legacy)
            fileArray = data.value("shared_files").toArray();
            totalFiles = data.value("total_files").toInt();
            currentPage = data.value("current_page").toInt();
            totalPages = data.value("total_pages").toInt();
        } else {
            // Direct array format (current API spec)
            QJsonDocument doc(data);
            if (doc.isArray()) {
                fileArray = doc.array();
            } else {
                // If the response is wrapped in a root object, try to find the array
                fileArray = data.value("data").toArray();
                if (fileArray.isEmpty()) {
                    // Last resort: check if the data itself contains the array
                    for (auto it = data.begin(); it != data.end(); ++it) {
                        if (it.value().isArray()) {
                            fileArray = it.value().toArray();
                            break;
                        }
                    }
                }
            }
            totalFiles = fileArray.size();
            currentPage = 1;
            totalPages = 1;
        }
        
        // Process shared files with proper typing
        QList<MvcSharedFileInfo> sharedFiles;
        QList<MvcFileInfo> legacyFiles; // For backward compatibility
        ...
        for (const QJsonValue& value : fileArray) {
            ...
            info.fileId = obj.value("file_id").toString();
            ...
            legacyFiles.append(info); // Add to legacy list for backward compatibility
        }
        
        std::cout << "📋 FILESERVICE: Emitting " << sharedFiles.size() << " shared files to UI" << std::endl;
        
        // Emit both signals for compatibility
        emit sharedFileListReceivedTyped(sharedFiles, totalFiles, currentPage, totalPages);
        emit sharedFileListReceived(legacyFiles, totalFiles, currentPage, totalPages);
        
    } else {
        // For owned files, use the standard wrapped format
        ...
        std::cout << "📋 FILESERVICE: Processing " << fileArray.size() << " owned files" << std::endl;
        ...
        std::cout << "📋 FILESERVICE: Emitting " << files.size() << " owned files to UI" << std::endl;
...
void FileService::handleAccessResponse(const QJsonObject& data) {
    QString fileId = data.value("file_id").toString();  // Use file_id instead of filename
    ...
    if (endpoint.endsWith("/access")) {
        ...
        emit usersWithAccessReceived(fileId, users);  // Use fileId
    }
    else if (endpoint.contains("/grant") || endpoint == "/api/files/share") {
        QString username = data.value("recipient_username").toString();  // Use recipient_username
        emit accessGranted(success, fileId, username);  // Use fileId
    }
    else if (endpoint.contains("/revoke")) {
        QString username = data.value("username").toString();
        emit accessRevoked(success, fileId, username);  // Use fileId
    }
}
